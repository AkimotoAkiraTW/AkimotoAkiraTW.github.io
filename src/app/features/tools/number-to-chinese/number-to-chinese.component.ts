...
export class NumberToChineseComponent {
  ...
  handleInput(value: string) {
    ...
    const intPartStr = parts[0].replace('-', '');
    const prefix = isNegative ? '-' : '';
    if (intPartStr.length > 15) {
      parts[0] = prefix + intPartStr.slice(0, 15);
  ...
  private convertToChinese(numStr: string): string {
    ...
    let integerStr = parts[0].replace(/^-/, '');
    ...
    if (integerStr === '0' || integerStr === '') {
      ...
      for (let i = 0; i < integerStr.length; i++) {
        ...
        if (d === 0) {
          ...
          if (zeroCount > 0) segmentResult += '零';
          zeroCount = 0;
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (d > 0) decimalResult += digits[d] + fractionUnits[i];
    }
    if (decimalResult === '') result += '整';
    else result += decimalResult;
    ...
    if (isNegative && finalResult !== '零元整' && finalResult !== '') {
      finalResult = '負' + finalResult;
    }
    return finalResult;
  }
}
